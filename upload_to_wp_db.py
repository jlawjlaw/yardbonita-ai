...
def main():
    ...
    for row in articles:
        ...
        # Insert into published table if not already present
        conn2 = sqlite3.connect(DB_PATH)
        rowcount = insert_into_published(conn2, row)
        conn2.close()

def insert_into_published(conn, row):
    cursor = conn.cursor()
    fields_to_copy = [
        'uuid', 'post_title', 'city', 'category', 'publish_date', 'published_url',
        'author', 'tier', 'seo_title', 'seo_description', 'focus_keyphrase', 'tags', 'slug'
    ]
    values = tuple(row.get(field, "") for field in fields_to_copy)

    cursor.execute(f"""
        INSERT OR IGNORE INTO published ({', '.join(fields_to_copy)})
        VALUES ({', '.join(['?'] * len(fields_to_copy))})
    """, values)
    conn.commit()
